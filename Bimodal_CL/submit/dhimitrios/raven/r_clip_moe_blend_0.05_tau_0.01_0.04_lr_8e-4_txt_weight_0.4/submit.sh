...
SIM_BLEND_RATIO=0.05

DESC=R_CLIP_MoE_TXT_${SIM_BASED_LOSS_ALPHA}_LR_${LR}_LOSS_WEIGHT_${T2T_LOSS_WEIGHT}_BLEND_RATIO_${SIM_BLEND_RATIO}

CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=4820 \
    --use_env clip.py \
    --run_name $DESC \
    --data $DATA \
    --output_dir /ptmp/dduka/work/training_metadata/bimodal_cl/dhimitrios/r_clip_moe_blend_0.05_tau_0.01_0.04_lr_8e-4_txt_weight_0.4 \
    --init_model \
    --use_amp \
    --epochs 30 --lr $LR \
    --ita_type $ITA_TYPE \
