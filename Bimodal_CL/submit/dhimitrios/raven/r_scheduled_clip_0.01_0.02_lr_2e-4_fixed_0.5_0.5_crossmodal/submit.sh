...
DESC=R_SCHEDULED_CLIP_${BASE_TAU}_${ALPHA}_${LR}_FIXED_0.5_0.5_CROSSMODAL

CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=4820 \
    --use_env clip.py \
    --run_name $DESC \ 
    --data $DATA \
    --output_dir /ptmp/dduka/work/training_metadata/bimodal_cl/dhimitrios/r_scheduled_clip_0.01_0.02_lr_2e-4_fixed_0.5_0.5_crossmodal/ \
    --init_model \
    --use_amp \
    --epochs 30 --lr $LR \
    --ita_type $ITA_TYPE \
