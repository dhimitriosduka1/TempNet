...
DESC=R_SOGCLR_FIXED_${TAU_MIN}_${TAU_MAX}_${LR}_I2I_${I2I_LOSS_WEIGHT}_T2T_${T2T_LOSS_WEIGHT}_UNIMODAL_AUGMENTED

CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=4820 \
    --use_env clip.py \
    --run_name $DESC \
    --data $DATA \
    --output_dir /ptmp/dduka/work/training_metadata/bimodal_cl/dhimitrios/r_sogclr_fixed_0.01_0.05_lr_2e-4_i2i_0.5_t2t_0.2_quad_crossmodal_and_unimodal_augmented/ \
    --init_model \
    --use_amp \
    --epochs 30 --lr $LR \
    --ita_type $ITA_TYPE \
    --temperature_scheduler fixed \
    --temp $TAU_MIN \
    --enable_i2i_loss \
    --enable_t2t_loss \
