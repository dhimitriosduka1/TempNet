...
PROJECT_DIR="/u/dduka/work/projects/TeMo/TempNet/Bimodal_CL"
cd "${PROJECT_DIR}"

DATA=cc3m
LR=2e-4
ITA_TYPE=dystress_loss

BASE_TAU=0.01
ALPHA=0.04
 
DESC=R_DYSTRESS_${BASE_TAU}_${ALPHA}_${LR}_SHIFTED

CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=4820 \
    --use_env clip.py \
    --run_name $DESC \
    --data $DATA \
    --output_dir /ptmp/dduka/work/training_metadata/bimodal_cl/dhimitrios/r_dystress_0.01_0.04_lr_2e-4_SHIFTED/ \
    --init_model \
    --use_amp \
    --epochs 30 \
    --lr $LR \
