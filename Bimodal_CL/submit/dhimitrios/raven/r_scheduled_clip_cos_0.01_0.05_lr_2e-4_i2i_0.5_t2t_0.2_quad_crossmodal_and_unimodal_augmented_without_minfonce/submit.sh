...
ITA_TYPE=scheduled_crossmodal_cosine_clip_with_augmentations_and_unimodal_loss

DESC=R_SCHEDULED_CLIP_COS_${TAU_MIN}_${TAU_MAX}_${LR}_I2I_${I2I_LOSS_WEIGHT}_T2T_${T2T_LOSS_WEIGHT}_QUAD_CROSSMODAL_AND_UNIMODAL_AUGMENTED_WITHOUT_MINFONCE

CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=4820 \
    --use_env clip.py \
    --run_name $DESC \
    --data $DATA \
    --output_dir /ptmp/dduka/work/training_metadata/bimodal_cl/dhimitrios/r_scheduled_clip_cos_0.01_0.05_lr_2e-4_i2i_0.5_t2t_0.2_quad_crossmodal_and_unimodal_augmented_without_minfonce/ \
    --init_model \
    --use_amp \
    --epochs 30 --lr $LR \
    --ita_type $ITA_TYPE \
    ...
    --exclude_modulated_info_nce_loss \