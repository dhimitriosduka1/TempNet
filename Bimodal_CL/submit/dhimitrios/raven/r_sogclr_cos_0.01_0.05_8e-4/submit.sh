...
DESC=R_SOGCLR_COS_${TAU_MIN}_${TAU_MAX}_LR_${LR}_GAMMA_${GAMMA}

CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=4821 \
    --use_env clip.py \
    --run_name $DESC \
    --data $DATA \
    --output_dir /ptmp/dduka/work/training_metadata/bimodal_cl/dhimitrios/r_sogclr_cos_0.01_0.05_8e-4/ \
    --init_model \
    --use_amp \
    --epochs 30 --lr $LR \
    --ita_type sogclr \
