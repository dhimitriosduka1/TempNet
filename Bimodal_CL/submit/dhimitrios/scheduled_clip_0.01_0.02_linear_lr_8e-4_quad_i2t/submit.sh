...
DESC=SCHEDULED_CLIP_${BASE_TAU}_${ALPHA}_LINEAR_${LR}_QUAD_I2T

CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=4820 \
    --use_env clip.py \
    --run_name $DESC \
    --data $DATA \
    --output_dir /BS/dduka/work/training_metadata/bimodal_cl/dhimitrios/scheduled_clip_0.01_0.02_linear_lr_8e-4_quad_i2t/ \
    --init_model \
    --use_amp \
    --epochs 30 --lr $LR \
    --ita_type $ITA_TYPE \
