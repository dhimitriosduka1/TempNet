...
DESC=BASELINE_CLIP_COS_${TAU_MIN}_${TAU_MAX}_LR_${LR}_RESUMED
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 --master_port=4820 \
    ...
    --data_path $DATA_PATH \
    --data $DATA \
    --output_dir /BS/dduka/work/projects/TempNet/Bimodal_CL/submit/dhimitrios/clip_cos_0.01_0.07_lr_8e-4 \
    --init_model \
    --use_amp \
    --epochs 30 --lr $LR \
    --ita_type clip \
    ...
    --tau_min $TAU_MIN \
    --tau_max $TAU_MAX \
    --checkpoint /BS/dduka/work/projects/TempNet/Bimodal_CL/output/BASELINE_CLIP_COS_0.01_0.07/2025-03-11_00-28/checkpoint_best.pth
